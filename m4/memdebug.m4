...
dnl adl_ENABLE_MEM_DEBUG	hanle the --enable-mem-debug option
...
AC_DEFUN([adl_CHECK_DMALLOC],
 [# see if dmalloc is usable
  AC_CHECK_LIB([dmalloc],[malloc],
   ...
   [ifelse([$2],,[AC_MSG_ERROR([Cannot find dmalloc])],[:])])])
...
AC_DEFUN([adl_ENABLE_MEM_DEBUG],
 [mallocdbg=""
  ...
  # were given (assume that adl_WITH_DMALLOC or adl_WITH_EFENCE
  # has already run).
  if test x"${with_dmalloc-no}" = xno -a x"${with_efence-no}" = xno; then
    ...
    [AC_HELP_STRING([--enable-mem-debug],
     ...
     [adl_CHECK_DMALLOC(,
       [adl_CHECK_EFENCE(,[mallocdbg="\<none\>"])])])
  fi])
