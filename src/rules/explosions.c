...
allocate_explosions (a_level_state *state, const a_level *lvl)
{
  ...
  XMALLOC_ARRAY (state->square_explo_type, lvl->square_count);
  bits->explo_list_max = 64;
  bits->explo_list_first_unused = 0;
  XMALLOC_ARRAY (bits->explo_list, bits->explo_list_max);
  bits->explo_timer = new_htimer (T_GLOBAL, HZ (70));
  bits->explo_time = 0;
}

void
release_explosions (a_level_state *state)
{
  a_level_state_bits *bits = state->private;
  free_htimer (bits->explo_timer);
  XFREE0 (state->square_explo_state);
  XFREE0 (state->square_explo_type);
  XFREE0 (bits->explo_list);
}
...
static int
compute_explosion_state (a_level_state *state, long orig_time)
{
  a_level_state_bits *bits = state->private;

  /* Elapsed time in "frames".  */
  int ef = (bits->explo_time - orig_time) / EXPLOSION_SLICES_PER_FRAMES;
  assert (ef >= 0);
			   ...
			   a_square_index idx, long orig_time)
{
  ...
  assert (bits->explo_time >= orig_time);

  explo_state = compute_explosion_state (state, orig_time);
  ...
  /* Insert the explosion into the list of active explosions
     only if it's active...  */
  if (explo_state >= 0) {
    if (bits->explo_list_max <= bits->explo_list_first_unused) {
      bits->explo_list_max += 64;
      XREALLOC_ARRAY (bits->explo_list, bits->explo_list_max);
    }

    bits->explo_list[bits->explo_list_first_unused].orig_time = orig_time;
    bits->explo_list[bits->explo_list_first_unused].idx = idx;
    bits->explo_list[bits->explo_list_first_unused].neighb_done = propagated;
    ++bits->explo_list_first_unused;

    /* Update level rendering information.  */
    state->square_explo_state[idx] = explo_state;
    state->square_explo_type[idx] = rand () % NBR_EXPLOSION_KINDS;
  }
}
...
update_explosions (a_level_state *state, const a_level *lvl)
{
  ...
  */
  ...
  bits->explo_time = read_htimer (bits->explo_timer);

  if (bits->explo_list_first_unused)
    dmsg (D_MISC, "explosions: %d/%d",
	  bits->explo_list_first_unused, bits->explo_list_max);

  shift_src = -1;
  shift_dest = 0;
  for (i = 0; i < bits->explo_list_first_unused; ++i) {
    a_square_index idx = bits->explo_list[i].idx;
    int explo_state = compute_explosion_state (state,
	...
	shift_src = i;
      ...
      state->square_explo_state[idx] = EXPLOSION_UNTRIGGERED;
...
#define SHIFT_ACTIVATED_EXPLOSIONS				\
	...
	memmove (bits->explo_list + shift_dest,			\
		 bits->explo_list + shift_src,			\
		 activated_qty * sizeof (*bits->explo_list));	\
  ...
  SHIFT_ACTIVATED_EXPLOSIONS;
  bits->explo_list_first_unused = shift_dest;
}
